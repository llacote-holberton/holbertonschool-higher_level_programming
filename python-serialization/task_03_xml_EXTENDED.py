...
def serialize_to_xml(dictionary, filename):
    # Arbitrarily defining the name of xml root tag as "data".
...
def deserialize_from_xml(filename):
    ...
    try:
        ...
        print(f"@dev object is of type {xmlTree.__class__.__name__}")
        print(xmlTree)
        print(xmlTree.getroot()[0])
        # Going for single level first to understand xml lib methods.
        treeRoot = xmlTree.getroot()
        print(dir(treeRoot))
        for child in treeRoot:
            print("New child found: ", child)
    ...
    for key, value in dictionary.items():
        ...
        if key.startswith('@'):
            ...
            if isinstance(value, list):
                ...
                # Child with attributes (ex: neighbor)
                subElement = ET.SubElement(xmlElement, key)
                recursiveDictToXml(value, subElement)
            else:
                subElement = ET.SubElement(xmlElement, key)
...
def serialize_to_multilevel_xml(dictionary, filename):
    ...
    print(f"@dev: created root Element {root}")
    print(f"@dev: dictonary is this: \n-------\n", dictionary, "\n-------\n")

    # Not only useless but actually creates duplicated subdata!!!
    # for key, value in dictionary.items():
    #     print(f"@dev Processing {key} : {value}")
    #     recursiveDictToXml(dictionary, root)
    # print("@dev Final STATE of xml tree")
    recursiveDictToXml(dictionary, root)
    # print(root, f"number of children: {len(root)}")
...
def recursiveXmlToDict(xmlElement: ET.Element) -> dict:
    ...
    print(f"@dev: Processing element {xmlElement.tag}")
    xml_as_dict = {}
    # NOTE: processing attributes must NOT be here because
    #   it is processed differently depending on whether
    #   current Element has children or not.

    # Element class implements __len__ magic method, returns number of children
    if len(xmlElement) > 0:
        # THANKS CLAUDE for helping me understand where to put that bit.
        ...
        for child in xmlElement:
            # print(f"@dev: processing sub {child.tag} at pos {processed}")
            if len(child) > 0:
                # Drill down recursively
                # CANNOT WORK as I would overwrite each sibling with the next
                # xml_as_dict[xmlElement.tag] = recursiveXmlToDict(child)
                # Dict have unique keys. Must use a list to regroup siblings.
                # Thanks Claude for that guard xd
                if child.tag not in xml_as_dict:
                    xml_as_dict[child.tag] = []
                xml_as_dict[child.tag].append(recursiveXmlToDict(child))
            else:
                # This is "bottom level", branching if/not has attributes
                if child.attrib:
                    xml_as_dict[child.tag] = {}
                    for attr_key, attr_val in child.attrib.items():
                        xml_as_dict[child.tag][f"@{attr_key}"] = attr_val
                else:
        ...
        if xmlElement.attrib:
            # Element has attributes so we must make them "as a dedicated dict"
...
def deserialize_from_multilevel_xml(filename):
    ...
    try:
        # with (filename) as xml_source_file:
        #     xmlTreeRoot = ET.parse(xml_source_file.read())
        #     print(f"@dev object is of type {xmlTreeRoot.__class__.__name__}")
        #     print(xmlTreeRoot)
        #     print(dict(xmlTreeRoot))
        xmlTree = ET.parse(filename)
        # print(f"@dev object is of type {xmlTree.__class__.__name__}")
        # print(xmlTree)
        # print(xmlTree.getroot()[0])
        # Going for the recursive although I know it's overkill here.
        # Because I want to know how to handle more complex structures.
        # print(dir(xmlTree))
        # test_dict = recursiveXmlToDict(xmlTree.getroot())
        # print(test_dict)
